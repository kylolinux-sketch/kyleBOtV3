    ...
    try {
      ...
      if (args[0] && args[0].toLowerCase() === 'setwall') {
        if (!messageReply || !messageReply.attachments || messageReply.attachments.length === 0) {
          return message.reply("❌ You must reply to an image to set the wallpaper.");
        }
        ...
        try {
          ...
          wallpaper = wallPath;
          return message.reply("✅ Custom wallpaper set successfully!");
        } catch (error) {
          console.error("Error downloading wallpaper:", error);
          return message.reply("❌ Unable to set the wallpaper.");
        }
      }
      ...
      if (args[0] && args[0].toLowerCase() === 'themes') {
        ...
        return message.reply(
          getLang("themeList", count, themeList) +
          "\n" +
      ...
      if (args[0] && args[0].toLowerCase() === 'createtheme') {
        ...
        if (success) return message.reply(`✅ Custom theme "${themeName}" created successfully!`);
        else return message.reply("❌ Error creating the theme.");
      }
      ...
      for (let i = 0; i < 24; i++) {
        hours24.push(Math.floor(Math.floor(totalMessages / 30) * 0.5 * (0.5 + Math.random() * 1.2)));
      }
      ...
      await generateDashboard(
        { uid, totalMessages, trend30, hours24, peak, breakdown, name: userData.name },
        message,
        usersData
      );
...
async function generateDashboard(data, message, usersData) {
  ...
  if (wallpaper && fs.existsSync(wallpaper)) {
    ...
    bg.addColorStop(0, "#0A0F29");
    bg.addColorStop(1, "#0E1335");
    ctx.fillStyle = bg;
  ...
  ctx.fillText(data.name, WIDTH / 2, 150);
  ...
  ctx.font = "bold 32px Arial";
  ctx.fillStyle = "#fff";
  ctx.fillText("30-DAY ACTIVITY TREND", WIDTH / 2, 540);
  ...
  ctx.font = "bold 32px Arial";
  ctx.fillStyle = "#fff";
  ctx.fillText("24-HOUR ACTIVITY PATTERN", WIDTH / 2, 900);
