...
async function getSimReply(api, event, prompt) {
  ...
  const apiBase = "https://norch-project.gleeze.com/api/sim";
...
module.exports = {
  config: {
    ...
    role: 0,
    category: "ai",
    guide: "sim [on | off | message]: Enable/disable auto-reply or send a message"
  },
  ...
  onStart: async function ({ api, event, args }) {
    ...
    if (action === "on") {
      ...
      return api.sendMessage(
        "✅ Sim auto-reply is now ON. I'll respond to all messages automatically.",
        event.threadID,
    ...
    if (action === "off") {
      simEnabled = false;
      return api.sendMessage(
        "❌ Sim auto-reply is now OFF. Use -sim <message> to ask me.",
        event.threadID,
        String(event.messageID)
      );
    }
    ...
    if (reply) {
      ...
      return api.sendMessage(
        "⚠️ Sim is currently unavailable.",
        event.threadID,
        String(event.messageID)
      );
    }
