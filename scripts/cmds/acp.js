...
module.exports = {
  config: {
    ...
    longDescription: "Accept or decline friend requests",
    category: "utility",
    guide: {
  ...
  onReply: async function ({ message, Reply, event, api, commandName }) {
    ...
    for (const stt of targetIDs) {
      ...
      if (!user) {
        failed.push(`🚫 Unable to find request #${stt}`);
        continue;
      }
    ...
    else api.sendMessage("❌ No valid request was processed.", event.threadID);
  ...
  onStart: async function ({ event, api, commandName }) {
    try {
      ...
      msg += "❌ del <number> — Decline request\n";
      msg += "💫 add all — Accept all\n";
      msg += "🔥 del all — Decline all\n\n";
      msg += "⏳ This menu will automatically be deleted in 2 minutes.\n";
      msg += "╚═══════════════════╝";
      ...
      console.error(error);
      api.sendMessage("❌ An error occurred while retrieving friend requests.", event.threadID);
    }
  }
};
