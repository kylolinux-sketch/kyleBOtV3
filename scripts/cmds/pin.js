...
async function generatePinterestCanvas(imageObjects, query, page, totalPages) {
  ...
  ctx.font = '24px Arial';
  ctx.textAlign = 'left';
  ctx.fillText('🔍 Pinterest Search', 20, 45);

  ctx.font = '16px Arial';
  ctx.fillStyle = '#b0b0b0';
  ctx.fillText(
    `Search results for "${query}", displaying up to ${imageObjects.length} images.`,
    20,
    75
  );
  ...
  const loadedPairs = await Promise.all(
    imageObjects.map(obj =>
      loadImage(obj.url)
        ...
        .catch(e => {
          console.error(`Unable to load image: ${obj.url}`, e && e.message);
          return null;
        })
    )
  );
...
module.exports = {
  config: {
    ...
    longDescription: "Searches for images on Pinterest with a canvas preview for navigation.",
    category: "Image",
    guide: {
      en:
        "{pn} query [-count]\n" +
        "• If count is used, images are sent directly.\n" +
        "• Without count, an interactive canvas view is displayed.\n" +
    ...
    try {
      ...
      if (count) {
        ...
        const streams = await Promise.all(
          urls.map(url => getStreamFromURL(url).catch(() => null))
        );
        ...
        const imagesForPage1 = allImageUrls
        ...
        const { outputPath: canvasPath, displayedMap } =
          await generatePinterestCanvas(imagesForPage1, query, 1, totalPages);

        const sentMessage = await message.reply({
          body:
            `🔖 𝗣𝗜𝗡𝗧𝗘𝗥𝗘𝗦𝗧 𝗖𝗠𝗗:\n▬▬▬▬▬▬▬▬▬▬▬\n`+
            `🖼️ ${allImageUrls.length} images found for "${query}".\n` +
            `Reply with a number (shown on the canvas) to get the image, or “next” for more.`,
      ...
      console.error(error);
      if (processingMessage) {
        try { await message.unsend(processingMessage.messageID); } catch {}
      }
      message.reply("An error occurred. The server or API may be unavailable.");
    }
  },
  ...
  onReply: async function({ api, event, message, Reply }) {
    try {
      if (!Reply)
        return message.reply("Session expired. Please run the command again.");
      ...
      if (input === 'next') {
        ...
        const imagesForNextPage = allImageUrls
        ...
        const { outputPath: canvasPath, displayedMap: nextDisplayedMap } =
          await generatePinterestCanvas(imagesForNextPage, query, nextPage, totalPages);

        const sentMessage = await message.reply({
          body:
            `🖼️ Page ${nextPage}/${totalPages}.\n` +
            `Reply with a number (from the canvas) to get the image, or “next” to continue.`,
          attachment: fs.createReadStream(canvasPath)
        });
        ...
        if (!isNaN(number) && number > 0) {
          if (number > displayCount) {
            return message.reply(
              `Invalid number. The current canvas shows only ${displayCount} image(s).`
            );
          }
          ...
          if (!stream)
            return message.reply("Unable to retrieve the requested image.");
          ...
          return message.reply(
            `Reply with a number (from the canvas) to get the image, or “next” to load more pages.`
          );
        }
      }
    } catch (error) {
