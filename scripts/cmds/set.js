module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData }) {
    try {
      const ADMIN_UIDS = ["61565434041712", "61565434041712"];
      ...
      switch (action) {
        case 'money':
          ...
          return api.sendMessage(`💰 Money set to ${amount} for ${userData.name}`, event.threadID);

        case 'exp':
          if (isNaN(amount)) return api.sendMessage("❌ Invalid amount", event.threadID);
          await usersData.set(targetID, { exp: amount });
          return api.sendMessage(`🌟 Experience set to ${amount} for ${userData.name}`, event.threadID);
        ...
        case 'custom':
          ...
          return api.sendMessage(`🔧 Variable ${variable} set to ${value} for ${userData.name}`, event.threadID);
