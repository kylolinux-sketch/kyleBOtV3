...
exports.config = {
    ...
    role: 0,
    countDown: 40,
    description: "Play the maze game with adjustable difficulty.",
    version: "1.0.3",
...
function generateMazeImage(difficulty = 15, grid = null, cols = null, highlightPath = null, wrongPath = null, currentPosition = null, progressPath = null) {
    ...
    ctx.fillRect(start.x * cellSize, start.y * cellSize, cellSize);
    ctx.fillStyle = "#000000";
    ctx.fillText("A", start.x * cellSize + cellSize / 2, start.y * cellSize + cellSize / 2);

    ctx.fillStyle = "#f28b82";
    ctx.fillRect(end.x * cellSize, end.y * cellSize, cellSize);
    ctx.fillStyle = "#000000";
...
exports.onReply = async ({ message, event, Reply, usersData }) => {
    ...
    if (!/^[urdl⬆️➡️⬇️⬅️]+$/i.test(userEmoji)) {
        return message.reply(`Use only valid movement emojis (⬆️ ➡️ ⬇️ ⬅️) or letters (u, r, d, l) in a single sequence.`);
    }
    ...
    if (isCorrectContinuation && userPath.length === solutionPath.length) {
        ...
        try {
            ...
            message.reply(`🎉 You solved the maze! (Error adding reward: ${e.message})`);
            global.GoatBot.onReply.delete(event.messageID);
        }
        return;
    }
    ...
    if (Reply.attempts >= 3) {
        ...
        await message.reply({
            body: `❌ Game over! You used all your attempts.\n\n✅ The correct path is shown in green\n❌ Your wrong path is shown in red`,
            attachment: fs.createReadStream(imagePath)
        });
        fs.unlinkSync(imagePath);
        global.GoatBot.onReply.delete(event.messageID);
    } else {
        await message.reply(`❌ Wrong path or movement! Try again from your last point.\n\n🔄 Attempts left: ${3 - Reply.attempts}`);
    }
};
