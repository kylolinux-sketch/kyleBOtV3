...
module.exports = {
  config: {
    ...
    countDown: 20,
    longDescription: {
      en: "Quickly generates AI images with Fluxx (Christus API)."
    },
    ...
    message.reply("⚡ Fluxx is generating your images. Please wait...", async () => {
      try {
        ...
        if (!response?.data?.status || !Array.isArray(response.data.images) || response.data.images.length !== 4) {
          api.setMessageReaction("❌", event.messageID, () => {}, true);
          return message.reply("❌ Image generation failed. Try another prompt.");
        }
  ...
  onReply: async function ({ api, event, Reply, message }) {
    ...
    try {
      ...
      message.reply({
        body: `🖼️ Here is your selected Fluxx image (U${index + 1}).`,
